...
$("#permission_add_sb").click(function () {
    $('#addteacher').data('bootstrapValidator').validate();//启用验证
    flag = $('#addteacher').data('bootstrapValidator').isValid();
    if (flag == true) {
        ...
        debugger;
        formData.append("teacherdId", jobsub.id);
        formData.append("teacherName", jobsub.teacherName);
        formData.append("text", jobsub.text);
        formData.append("school", jobsub.school);
        formData.append("nationality", jobsub.nationality);
        formData.append("photo_yourself_from", $("input[name='photo_yourself']")[0].files[0]);
        formData.append("personal_video", $("input[name='personal_video']")[0].files[0]);
        $.ajax({
            //几个参数需要注意一下
            type: "POST",//方法类型
            // contentType : 'application/x-www-form-urlencoded;charset=utf-8',
            //  dataType: "json",//预期服务器返回的数据类型
            url: "/teacher/teacherAdd",//url
    ...
    dataTable = $('#myTable').DataTable({
        ...
        "ajax": {
            ...
            "data": function (d) {
                ...
                if (formMap.teacherName != "")
                    d.teacherName = formMap.teacherName;
        ...
        columns: [
...

            {
                ...
                render: function (data, type, row) {
                    ...
                    return "<a onclick='picture(" + data.fileId + ")'><span style='font-size: 26px;' class='glyphicon glyphicon-search'></span></a>";
                }
            },
            {
                data: 'video',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    switch (data) {
                        case 0:
                            return '<button class=" btn btn-default btn-sm" onclick="showTeacher(' + row.teacherdId + ',1)">否</button>';

                            break;
                        case 1:
                            return '<button class=" btn btn-success btn-sm" onclick="showTeacher(' + row.teacherdId + ',0)">是</button>';

                            break;
                        default:
                            return '<button class=" btn btn-danger btn-sm" onclick="showTeacher(' + row.teacherdId + ',0)">异常</button>';
            ...
            },
            {
                ...
                "orderable": false, // 禁用排序
                render: function (data, type, row) {
                    return '<button class=" btn btn-primary btn-sm" onclick="onreload(' + data + ',' + row.nationality + ')">课表</button>&nbsp;' +
                        '<button class=" btn btn-warning btn-sm" onclick="updatejob(' + encodeURI(row.teacherdId) + ',' + encodeURI(row.nationality )+ ',\'' + encodeURI(row.teacherName) + '\',\'' + encodeURI(row.text) + '\',\'' + encodeURI(row.school) + '\')">修改</button>';
                }
            }
        ]
    });
...
function updatejob(teacherdId, nationality, teacherName, text, school) {
    debugger;
    jobsub.id = decodeURI(teacherdId);
    jobsub.nationality = decodeURI(nationality);
    jobsub.teacherName = decodeURI(teacherName);
    jobsub.text = decodeURI(text);
    jobsub.school = decodeURI(school);
  /*  var fileurl = ["/file/picture?fileid=49"];
    var fileconfig = [ {caption: "People-1.jpg", size: 576237, width: "120px", url: "/site/file-delete", key: 1}];
*/
    // initFileInput("tearcherPicture",fileurl,"");
    // initVideoInput("teacherVideo","","");
    $('#addteacher').modal('show');
}

function onreload(teacherid, nationality) {
    debugger;
    $.ajax({
        "url": "/timetable/databyteacher",
        "type": "post",
        "data": {
            "teacherid": teacherid
        },
        "success": function (result) {
            if (result !== "") {
                ...
                $(".left-hand-TextDom").text("China time");
...
function showTeacher(teacherdId, statetype) {
    $.ajax({
        "url": "/teacher/updateShowTeacher",
        "type": "post",
        "data": {
...
function initVideoInput(ctrlName,fileurl,fileconfig) {
    var control = $('#' + ctrlName);
    var type = $("input[name='typeid']").val();
    control.fileinput({
        language: 'en', //设置语言
        uploadUrl: '',//'/document/uploadfile.do?type='+type, //上传的地址
        allowedFileExtensions: ["mp4", "avi", "dat", "3gp", "mov", "rmvb"], //'JPG', 'jpg', 'gif', 'png', 'exe', 'txt', 'rar', 'jar', 'bmp'接收的文件后缀
        //uploadExtraData:{"id": 1, "fileName":'123.mp3'},
        uploadAsync: true, //默认异步上传
        showUpload: false, //是否显示上传按钮
        showRemove: true, //显示移除按钮
        ...
        maxImageWidth: 10000,//图片的最大宽度
        maxImageHeight: 10000,//图片的最大高度
        maxFileSize: 1024 * 10 * 5, //单位为kb，如果为0表示不限制文件大小
        //minFileCount: 0,
