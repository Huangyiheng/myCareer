    ...
    if (id != null) {
        $.ajax({
            ...
            "success": function (result) {
                ...
                if (result !== "") {
                    jobsub.id = result.id;
                    jobsub.address = result.address;
                    jobsub.age = result.age;
          ...
          /*  { data: 'passwd' , title: '密码'},*/
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    switch (data) {
                        case 0:
                            return '<button class=" btn btn-default btn-sm" onclick="showJoins(' + row.id + ',1)">否</button>';

                            break;
                        case 1:
                            return '<button class=" btn btn-success btn-sm" onclick="showJoins(' + row.id + ',0)">是</button>';

                            break;
                        default:
                            return '<button class=" btn btn-danger btn-sm" onclick="showJoins(' + row.id + ',0)">异常</button>';
                ...
                "orderable": false, // 禁用排序
                render: function (data, type, row) {
                // <button class=" btn btn-success btn-sm" onclick="show(' + data + ')">查看</button>&nbsp;
                    return '<button class=" btn btn-warning btn-sm" onclick="update(' + data + ')">修改</button>';
                }
            }
        ]
    });
})
function showJoins(id, statetype) {
    $.ajax({
        "url": "/joins/updateShowJoins",
        "type": "post",
        "data": {
            "id": id,
            "showJoins": statetype,
