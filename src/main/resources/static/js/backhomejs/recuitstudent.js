function deletedata(e, dt, node, config, bt) {
    ...
    if (params.length < 1) {
        ...
        $.ajax({
            "url": "/recruitstudent/recruitstudent_delete",
            "type": "post",
            "data": {
                "IDArray": params
            },
...
$(function () { //DataTable必须写在匿名函数里面,否则会报错
    ...
    dataTable = $('#myTable').DataTable({
        ...
        "ajax": {
            "url": "/recruitstudent/recrui_student_data",
            "type": "POST",
            "data": function (d) {
          ...
          /*  { data: 'passwd' , title: '密码'},*/
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    switch (data) {
                        case 0:
                            return '<button class=" btn btn-default btn-sm" onclick="showStudent(' + row.id + ',1)">否</button>';

                            break;
                        case 1:
                            return '<button class=" btn btn-success btn-sm" onclick="showStudent(' + row.id + ',0)">是</button>';

                            break;
                        default:
                            return '<button class=" btn btn-danger btn-sm" onclick="showStudent(' + row.id + ',0)">异常</button>';
...
function showStudent(id, statetype) {
    $.ajax({
        "url": "/recruitstudent/updateShowStudent",
        "type": "post",
        "data": {
            "id": id,
            "showStudent": statetype,
        },
        "success": function (result) {
            debugger;
            if (result == "success") {
